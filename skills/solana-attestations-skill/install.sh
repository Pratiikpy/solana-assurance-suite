#!/bin/bash
# solana-attestations-skill - Standard Installer
set -e
...
SKILL_PATH="$SKILLS_DIR/solana-attestations"
...
echo -e "${CYAN}solana-attestations-skill${NC} — the SAS credential primitive, verified."
echo "Installs: solana-attestations → $SKILL_PATH (+ solana-dev core if missing)."
echo "Composes with solana-sybil-defense and solana-testing."
echo
...
echo -e "${CYAN}[2/2]${NC} Installing solana-attestations skill..."
...
echo "  • \"issue a proof-of-human attestation with SAS\""
echo "  • \"verify this attestation safely (on-chain + off-chain)\""
echo "  • \"gate my airdrop claim on a valid credential\""
echo
echo "Runnable verifier: tools/sas-verify/  ·  Proof: examples/attestation-verify/"
