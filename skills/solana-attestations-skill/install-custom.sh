#!/bin/bash
# solana-attestations-skill - Interactive Installer
set -e
...
echo -e "${CYAN}solana-attestations-skill — interactive install${NC}"; echo
echo "Install location?"
echo "  1) Personal  (~/.claude/skills/solana-attestations)   [default]"
echo "  2) Project   (./.claude/skills/solana-attestations)"
echo "  3) Custom path"
read -p "Choice [1]: " loc
case "${loc:-1}" in
...
SKILL_PATH="$SKILLS_DIR/solana-attestations"; CORE_SKILL_PATH="$SKILLS_DIR/solana-dev"
mkdir -p "$SKILLS_DIR"
