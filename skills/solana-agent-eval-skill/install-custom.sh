#!/bin/bash
# solana-agent-eval-skill - Interactive Installer
set -e
...
echo -e "${CYAN}solana-agent-eval-skill — interactive install${NC}"; echo
echo "Install location?"
echo "  1) Personal  (~/.claude/skills/solana-agent-eval)   [default]"
echo "  2) Project   (./.claude/skills/solana-agent-eval)"
echo "  3) Custom path"
read -p "Choice [1]: " loc
case "${loc:-1}" in
...
SKILL_PATH="$SKILLS_DIR/solana-agent-eval"; CORE_SKILL_PATH="$SKILLS_DIR/solana-dev"
mkdir -p "$SKILLS_DIR"
